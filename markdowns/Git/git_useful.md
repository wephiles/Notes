...
# 7. pycharm中的 `Amend commit` 的作用

在 PyCharm 的提交对话框（`Commit` 工具窗口，快捷键 `Alt+0`）中，勾选 "Amend" 选项后，`Commit` 按钮会变为 `Amend Commit`。

这时，你的**当前暂存区（Staged Changes）中的内容**会和**上一次提交**合并，形成一个全新的提交，来替换掉旧的提交。

它主要解决两种场景：

- **修改提交信息**：如果你发现上次提交的日志写错了，可以勾选 "Amend"，在提交信息框中修改文字，然后点击 `Amend Commit`。这会用新的提交信息替换旧的。
- **补充遗漏的修改**：如果你提交后才发现漏掉了某个文件或某处修改，可以先 `git add` 这些遗漏的内容，然后勾选 "Amend" 并点击 `Amend Commit`。这样，漏掉的修改就会被合并到上一次提交中，而不是生成一个独立的 "啊，忘了添加一个文件" 这样的新提交。

**⚠️ 重要提醒**：这个操作会**重写 Git 历史**。因此，**只应针对尚未推送到远程仓库的提交使用**。如果提交已经推送，强行修改会给协作者带来麻烦。

## ⌨️ 使用 Git 命令进行同等操作

PyCharm 的 "Amend commit" 功能，对应的 Git 命令是：
...
如果你的暂存区没有新内容，只是想修改上一次提交的日志，直接运行：
...
这会直接用 "新的提交信息" 替换掉旧的提交信息。

### 2. 补充遗漏的文件或修改

如果你已经提交了，但发现有文件忘了加进去，可以按以下步骤操作：
...
# 2. 使用 --amend 提交，将暂存区的内容合并到上一次提交
git commit --amend
```

同样，你也可以加上 `-m` 参数来一并修改提交信息。

------

## ⚠️ 注意事项

- **安全红线**：**永远不要**对已经 push 到远程仓库的公共分支（如 `main`、`master`）上的提交执行 `--amend`。这会改变提交的哈希值，导致他人本地历史与远程历史冲突。
- **权限问题**：如果远程分支被设置为"受保护分支"（Protected Branch），通常会禁止强制推送，你的修改将无法推送到远程。
- **Git 操作本质**：`git commit --amend` 并非在原有提交上“修改”，而是**生成一个全新的提交来替换它**。因此，它之后的所有提交的哈希值也都会随之改变。
